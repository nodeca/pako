...
class Deflate {
  ...
  /** Error message, if {@link Deflate.err} != 0. */
  msg: string;
   ...
   * (call {@link Deflate.push} with `Z_FINISH` / `true` param).
   */
  result?: Uint8Array;
   ...
   * free memory and fill {@link Deflate.result} / {@link Deflate.err} properties.
   *
   * @param status deflate status. 0 ({@link Z_OK}) on success, other if not.
   */
