...
import { Deflate, Inflate, deflate, inflate } from '../index.mjs';
...
describe('Dummy push (force end)', () => {
  ...
  it('deflate end', () => {
    ...
    assert.deepStrictEqual(deflator.result, deflate(data));
  });

  it('inflate end', () => {
    const data = deflate(samples.lorem_utf_100k);

    const inflator = new Inflate();
    inflator.push(data);

    assert.deepStrictEqual(inflator.result, inflate(data));
  });
